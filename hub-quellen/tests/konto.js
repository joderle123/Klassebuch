...
(async () => {
  ...
  check('Annexe sieht Klassenbuch, ELDiB, Toolbox, Skills-Kurs, Lernen', JSON.stringify(na.sort()) === JSON.stringify(['eldib', 'klassenbuch', 'lernen', 'skills-kurs', 'toolbox']), na.join(','));
  check('Kacheln passend (5)', (await page.$$('#home-body .tile[data-href]')).length === 5);
  ...
  na = await navApps();
  check('Diagnostique sieht Journal, Screening, ELDiB, Toolbox, Skills-Kurs, Lernen', JSON.stringify(na.sort()) === JSON.stringify(['eldib', 'journal', 'lernen', 'screening', 'skills-kurs', 'toolbox']), na.join(','));
  ...
  na = await navApps();
  check('ISA sieht Journal, ELDiB, Toolbox, Skills-Kurs, Lernen', JSON.stringify(na.sort()) === JSON.stringify(['eldib', 'journal', 'lernen', 'skills-kurs', 'toolbox']), na.join(','));
