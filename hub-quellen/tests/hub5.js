...
(async()=>{
  ...
  ok(names.join('|')==='Klassenbuch|ELDiB-Generator|Toolbox|Skills-Kurs|Lernen','Kacheln: '+names.join(', '));
  ...
  ok(await p.$$eval('.chip.tab',c=>c.length)===3,'"eigener Tab" bei Klassenbuch, Toolbox und Skills-Kurs');
  ...
  const [kuTab]=await Promise.all([ctx.waitForEvent('page'),p.click('.tile[data-tabtile="skills-kurs"] .tile-open')]);
  await kuTab.waitForLoadState('load'); await kuTab.waitForTimeout(2500);
  ok(/apps\/toolbox\.html#kurs$/.test(kuTab.url()),'Skills-Kurs öffnet die Toolbox mit #kurs: '+kuTab.url());
  ok(await kuTab.locator('.ku-held').count()===1,'Skills-Kurs zeigt die nächste Einheit');
  ...
  ok(kuTab.isClosed(),'Skills-Kurs-Tab geschlossen');
  ...
  ok(names.join('|')==='Journal|Befundbericht|ELDiB-Generator|Toolbox|Skills-Kurs|Lernen','Kacheln: '+names.join(', '));
