...
.gate-card.breit{""")
# 8) Annexe Junglinster: nur das Team der Annexe, ohne Datenbank und Journal (siehe annexe.py)
if os.path.exists(os.path.join(SP, 'annexe.py')):
    import importlib.util
    _spec = importlib.util.spec_from_file_location('annexe', os.path.join(SP, 'annexe.py'))
    _annexe = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(_annexe)
    s = _annexe.anpassen(s)
