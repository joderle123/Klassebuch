// Test: Klassenbuch (Testversion aus klassenbuch/quellen) – Rundgang durch alle Bereiche, keine Anfragen ins
// Internet, keine eingebauten Daten, Verknüpfungen zu Toolbox/Screening/Lernen im Hub, Screening-Reiter nur
...
(async () => {
  ...
  console.log('1) Start ohne eingebaute Daten, ohne Internet');
  ...
  check('Leere Klasse, keine eingebauten Einträge', st.roster === 0 && st.eintraege === 0, st);
  ...
  check('Datei unter 2 MB, Titel „Klassenbuch · CDSE“', st.groesse < 2e6 && st.titel === 'Klassenbuch · CDSE', st.groesse);
  ...
  check('Menü ohne „Material-Bibliothek“, „Pathologien“ und „KI-Anonymisierung“', !navs.includes('patho') && !navs.includes('ai') && !(await page.$('.kb-nav [data-kb-nav="material"]')), navs);
  const kaputt = [];
  ...
  check('Hinweis „Screening ist in den Hub umgezogen“ mit Knopf', scr.includes('in den CDSE Hub umgezogen') && !!(await page.$('.kb-scr-hinweis [data-kb-hub="screening"]')));
