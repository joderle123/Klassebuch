...
(async () => {
  ...
  /* Annexe: die eingebaute Klasse steht auf beiden Geräten, dazu die beiden erfundenen Kinder */
  const listeA = await A.page.evaluate(() => KB_ROSTER.list().map(s => s.name).sort().join()), listeB = await B.page.evaluate(() => KB_ROSTER.list().map(s => s.name).sort().join());
  check('Gerät B übernimmt die Klasse aus der Team-Datei', listeB === listeA && /Lea Beispiel/.test(listeB) && /Tom Muster/.test(listeB), { A: listeA, B: listeB });
  ...
  check('Team-Datei des Journals wird abgelehnt, nichts geschrieben', /keine Team-Datei des Klassenbuchs/.test(stC) && JSON.parse(DATEIEN['journal-team.json']).colls.pei.length === 0 && !JSON.parse(DATEIEN['journal-team.json'])._app, stC);
  ...
  /* Annexe: im Réunion-Reiter stehen die Ziele der nächsten Réunion im Eingabefeld (nächste Prüfung) */
  check('Übersicht, Verlauf und Réunion-Reiter zeigen die Zieltexte, nie „[object Object]“', [ueb, ver, reu].every(t => !t.includes('[object Object]')) && ueb.includes('Pünktlich kommen') && ver.includes('Hausaufgaben mitbringen'));
