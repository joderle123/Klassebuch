...
(async () => {
  ...
  console.log('1) Start (Annexe: mit der eingebauten Klasse)');
  check('Startseite ist die Heute-Tafel', await page.isVisible('#kb-heute.active'));
  ...
  /* Annexe: zur eingebauten Klasse kommen die drei erfundenen Kinder – Tom und Lea haben heute einen Eintrag */
  const oe = /(\d+) von (\d+) ohne Eintrag/.exec(t);
  check('Anwesenheit heute: Tom 2 Std. unentschuldigt, Lea entschuldigt, alle anderen ohne Eintrag', /Tom Muster\s?2 Std\. unentschuldigt/.test(t) && /Lea Beispiel\s?2 Std\. entschuldigt/.test(t) && !!oe && (+oe[2]) - (+oe[1]) === 2, t.slice(0, 400));
  check('Entschuldigung offen: Ben mit 12 Std. seit dem ersten Tag', /Ben Probe\s?12 Std\. seit/.test(t));
  /* mögliche Stunden für Ben (L2): jeder Schultag des Trimesters bis heute mit den Stunden laut Stundenplan (montags nur 5) */
  const erwartetP = Math.round(12 / (await page.evaluate(() => { const r = KB_TERMS.dateRange(), h = todayIso(), bis = h < r.to ? h : r.to, tt = KB_ANW.getTimetable().L2 || {}; let n = 0; for (const d = new Date(r.from + 'T12:00:00'); ; d.setDate(d.getDate() + 1)) { const iso = d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0'); if (iso > bis) { break; } const w = d.getDay(); if (w > 0 && w < 6 && !KB_ANW.holidayInfo(iso)) { KB_ANW.blocks().forEach((b, i) => { n += KB_ANW.hoursOf({ blockId: b.id, subject: (tt[w] || [])[i] }); }); } } return n; })) * 100);
