...
/* Stände von Apps, die nicht mehr in hub-apps.js stehen, fallen weg */
Object.keys(vers).forEach(function (k) {
  if (!(win.CDSE_APPS || []).some(function (a) { return a && a.id === k; })) { delete vers[k]; }
});
...
/* Annexe: die geholten Apps gleich wieder anpassen (Toolbox ohne „ISA“, Pathologien) */
if (fs.existsSync(path.join(ROOT, 'annexe-apps.cjs'))) {
  var an = cp.spawnSync(process.execPath, [path.join(ROOT, 'annexe-apps.cjs')], { stdio: 'inherit' });
  if (an.status !== 0) { fehler++; }
}
